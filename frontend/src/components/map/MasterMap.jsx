...
export default function MasterMap({
  ...
  const [activeFilter, setActiveFilter] = useState('ALL') // ALL | INCIDENTS | SHELTERS | TEAMS

  const selectedReport = reports.find((r) => r.id === selectedId)
  const selectedCoords = selectedReport?.coordinates

  const heatPoints = (reports || [])
    .filter((r) => r.coordinates && r.trustStatus !== 'BLACKLISTED')
    .map((r) => ({ lat: r.coordinates.lat, lng: r.coordinates.lng, weight: (r.trustScore ?? 50) / 100 }))
    ...
    <div className="relative h-full w-full map-shell overflow-hidden select-none">
      ...
      <div className="absolute top-4 left-4 z-[1000] flex flex-wrap items-center gap-2">
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-mono font-medium text-white shadow-lg backdrop-blur-md transition-all hover:bg-slate-700/90 active:scale-95"
      ...
      {/* Floating Tactical Legend Overlay (Bottom Left) */}
      <div
      ...
      >
        ...
        {/* Hazard Polygons & Peer Clusters (Feature 10) */}
        ...
        {/* Field Rescue Units (Boats, Medical Trauma, Drones, Trucks) */}
